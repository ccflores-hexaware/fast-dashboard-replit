...
export function useCMDBColumnVisibility(): UseCMDBColumnVisibilityReturn {
  ...
  useEffect(() => {
    if (Object.keys(columnVisibility).length > 0) {
      localStorage.setItem(`cmdb-column-visibility-${isAdmin ? 'admin' : 'viewer'}`, JSON.stringify(columnVisibility));
    }
  }, [columnVisibility, isAdmin]);

  useEffect(() => {
    if (Object.keys(cardFieldVisibility).length > 0) {
      localStorage.setItem('cmdb-card-field-visibility', JSON.stringify(cardFieldVisibility));
    }
  }, [cardFieldVisibility]);
